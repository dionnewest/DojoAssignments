...
def success(req):
	if 'user_id' not in req.session:
		return redirect ('/main')
	other_users = User.manager.exclude(id=req.session['user_id'])
	user = User.manager.get(id=req.session['user_id'])
	all_friendships = Friend.objects.all()
	users_friends = user.friend.all()
	friend_set = set()
	for friendships in users_friends:
		friend_set.add(friendships.friend)
	not_friends = set(other_users) - friend_set
	content = {
